...
def admin_showreqs():
    ...
    if current_user.is_admin == True:
        ...
        return render_template("admin_pendingrequests.html", posts=requests,
                               committees=committees)
    else:
        ...
        if string == 'True':
            ...
            if user.active:
                ...
                if login_user(user):
                    ...
                    return render_template('index.html',
                                           error="Could not login. System error.\
...
def savepost(postid):
    post = Requests.query.filter(Requests.id == postid).first()
    comments = Comments.query.filter(Comments.request_id == post.id)
    pdf = create_pdf(render_template('showpost.html', post=post,
                     comments=comments))
    return pdf
...
def logout():
    '''
    Logs a user out. (No need to pass the actual user,
    pops current_user out of session).
